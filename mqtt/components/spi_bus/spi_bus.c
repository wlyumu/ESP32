...
#define SPI_MUTEX_TICKS_TO_WAIT 2
...
#define SPI_DEVICE_MUTEX_TAKE(p_spi_dev, ret) if (!xSemaphoreTake((p_spi_dev)->mutex, SPI_MUTEX_TICKS_TO_WAIT)) { \
        ESP_LOGE(TAG, "spi device(%d) take mutex timeout, max wait = %d ticks", (int32_t)((p_spi_dev)->handle), SPI_MUTEX_TICKS_TO_WAIT); \
        return (ret); \
    }
...
{
    SPI_BUS_CHECK(SPI1_HOST < host_id && host_id <= SPI3_HOST, "Invalid spi host id", NULL)
    uint8_t index = host_id - 1;
    ...
    int dma_cha = host_id;//set dma channel equals to host_id by default
    esp_err_t  ret = spi_bus_initialize(host_id, &buscfg, dma_cha);
